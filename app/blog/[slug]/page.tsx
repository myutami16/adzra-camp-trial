...
export async function generateMetadata({ params }: BlogPostPageProps) {
  try {
    ...
    return {
      ...
      description: post.body?.slice(0, 160) || `Artikel ${post.title} dari Adzra Camp`,
    }
  } catch (error) {
    console.error("Error generating metadata:", error)
    return {
      title: "Adzra Camp Blog",
      description: "Blog Adzra Camp - Informasi dan tips tentang camping dan outdoor",
    }
  }
}
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  const safePost = post
    ? {
        ...
        thumbnailUrl: post.thumbnailUrl || "/placeholder.svg?height=400&width=800",
    ...
    : {
        ...
        slug: params.slug,
        body: error ? `<p>Error loading article: ${error.message}</p>` : "<p>Article could not be loaded.</p>",
  ...
  try {
    ...
    relatedPosts = relatedPostsData.data.content.filter((p) => p.id !== safePost.id).slice(0, 3)
  } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <Link href="/blog" className="inline-flex items-center text-gray-600 hover:text-primary-dark mb-6">
      ...
      <article className="max-w-3xl mx-auto">
        <div className="mb-8">
          ...
          <div className="flex items-center gap-4 text-gray-500 mb-6">
            <time dateTime={safePost.createdAt}>{formatDate(safePost.createdAt)}</time>
          ...
          {safePost.thumbnailUrl && (
            <div className="relative h-[300px] md:h-[400px] rounded-lg overflow-hidden mb-8">
              <Image
                src={safePost.thumbnailUrl || "/placeholder.svg"}
                alt={safePost.title}
                fill
                className="object-cover"
                onError={(e) => {
                  console.error("Image failed to load:", safePost.thumbnailUrl)
        ...
        <div className="prose max-w-none">
          {safePost.body ? <div dangerouslySetInnerHTML={{ __html: safePost.body }} /> : <p>Konten tidak tersedia.</p>}
        </div>
