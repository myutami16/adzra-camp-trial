...
export async function generateMetadata({ params }: ProductPageProps) {
  ...
  return {
    ...
    description: product.deskripsi || `Detail produk ${product.namaProduk} dari Adzra Camp`,
  }
}
...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  const filteredRelatedProducts = relatedProducts.data.products.filter((p) => p.id !== product.id).slice(0, 4)

  return (
    <div className="container mx-auto px-4 py-8">
      <Link href="/produk" className="inline-flex items-center text-gray-600 hover:text-primary-dark mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
        ...
        <div className="space-y-6">
          ...
          <div className="text-2xl font-bold text-primary-dark">{formatRupiah(product.harga)}</div>
          ...
          <div className="prose max-w-none">
            ...
            <p>{product.deskripsi || "Tidak ada deskripsi tersedia untuk produk ini."}</p>
          </div>
