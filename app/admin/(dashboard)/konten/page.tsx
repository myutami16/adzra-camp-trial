...
import { Loader2, Plus, MoreHorizontal, Pencil, Trash2, Search, Eye, RefreshCw } from "lucide-react"
...
export default function AdminContentPage() {
  ...
  const [contentToDelete, setContentToDelete] = useState<string | number | null>(null)
    ...
    try {
      ...
      if (Array.isArray(data.content) && data.content.length > 0) {
        ...
        if (process.env.NODE_ENV === "development") {
          ...
          setUseMockData(true)
          setError("Menggunakan data contoh karena API tidak tersedia. Ini hanya untuk mode pengembangan.")
        } else {
          setContent([])
          setError("Data konten tidak dalam format yang diharapkan atau tidak ada konten yang tersedia")
        }
      }
    } catch (error) {
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        setContent(MOCK_CONTENT)
        setUseMockData(true)
        setError("Gagal memuat data konten. Menggunakan data contoh untuk mode pengembangan.")
      } else {
        setContent([])
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500">Kelola artikel dan konten yang ditampilkan di website</p>
        </div>
      ...
      {error && (
        <div
          className={`border px-4 py-3 rounded-lg ${useMockData ? "bg-yellow-50 border-yellow-200 text-yellow-700" : "bg-red-50 border-red-200 text-red-700"}`}
        >
          {error}
        </div>
      )}
      ...
      <div className="border rounded-lg">
        {isLoading ? (
          ...
          <div className="text-center p-8 text-gray-500">
            {searchQuery ? "Tidak ada konten yang sesuai dengan pencarian" : "Belum ada konten"}
          </div>
        ) : (
          <Table>
            <TableHeader>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={() => router.push(`/admin/konten/edit/${item.id}`)}>
      ...
      <DeleteContentDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen} onConfirm={handleDeleteConfirm} />
    </div>
  )
}
