...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || "https://adzra-camp-store.vercel.app/api"
...
export const fetchAdminProducts = async (params = {}) => {
  try {
          ...
          if (result.success && Array.isArray(result.data)) {
            ...
            return {
              ...
              pagination: {
                ...
                totalItems: result.totalCount || result.count || mappedProducts.length,
    ...
    try {
      ...
      try {
        const result = await fetchWithAuth(endpoint)
        console.log("Admin products response from alternative endpoint:", result)
        ...
        console.error("Error in fetchWithAuth for /admin/products:", fetchError)
        // Continue to next attempt rather than throw
      }
    } catch (adminError) {
      console.error("Error fetching from alternative admin endpoint:", adminError)
    }
...
export const deleteProduct = async (id: number | string) => {
  try {
    console.log(`Attempting to delete product with ID: ${id} from primary endpoint`);
    ...
    try {
      ...
      console.log(`Successfully deleted product with ID ${id} from primary endpoint:`, result);
      return result;
    } catch (primaryError) {
      console.error(
      ...
      try {
        ...
        console.log(`Successfully deleted product with ID ${id} from first fallback endpoint:`, fallbackResult);
        ...
        try {
          ...
          console.log(`Successfully deleted product with ID ${id} from second fallback endpoint:`, fallbackResult2);
    ...
    console.error(`All attempts to delete product with ID ${id} failed:`, error);
    throw error;
  }
};
          ...
          if (result.success && Array.isArray(result.data)) {
            ...
            return {
              ...
              pagination: {
                currentPage: result.currentPage || 1,
                totalPages: result.totalPages || 1,
                totalItems: result.totalCount || result.count || mappedContent.length,
...
export const getContentById = async (id: number | string) => {
  try {
    ...
    try {
      ...
      console.log(`Content response for ID ${id} from fallback endpoint:`, data)
...
export const createContent = async (formData: FormData) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (!fallbackResponse.ok) {
        const error = await fallbackResponse.json().catch(() => ({ message: "An error occurred" }))
        throw new Error(error.message || `Request failed with status ${fallbackResponse.status}`)
      }
...
export const updateContent = async (id: number | string, formData: FormData) => {
  const token = getToken()
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log("First update attempt failed, trying fallback endpoint")
      const fallbackResponse = await fetch(`${API_BASE_URL}/admin/content?id=${id}`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
export const deleteContent = async (id: number | string) => {
  try {
    ...
    console.error(`Error deleting content with ID ${id} from first endpoint:`, error)

    // Try fallback endpoint
    try {
      return await fetchWithAuth(`/admin/content?id=${id}`, { method: "DELETE" })
    } catch (fallbackError) {
      console.error(`Error deleting content with ID ${id} from fallback endpoint:`, fallbackError)
      throw fallbackError
    }
  }
}
