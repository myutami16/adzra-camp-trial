...
export async function fetchProducts(
  ...
  try {
    ...
    if (result.data && result.data.products) {
      // Standard structure with data.products
      return result
    } else if (result.products) {
      // Alternative structure with products at root
      return {
        data: {
      ...
      return {
        data: {
          products: result,
          pagination: { currentPage: 1, totalPages: 1, totalItems: result.length, itemsPerPage: result.length },
        },
      }
...
export async function fetchContent(
  ...
  try {
    ...
    const url = `${API_BASE_URL}/content${queryParams.toString() ? `?${queryParams.toString()}` : ""}`
...
export async function getContentBySlug(slug: string): Promise<ContentItem | null> {
  try {
      ...
      if (directResponse.ok) {
        ...
        if (directResult.success && directResult.data) {
          ...
          return {
            ...
            body: directResult.data.description || directResult.data.summary || "<p>No content available.</p>",
    ...
    console.log("No content found with slug parameter, trying with path parameter")
    try {
...
export async function getProductBySlug(slug: string): Promise<Product | null> {
  try {
    ...
    console.log("No product found with slug parameter, trying with path parameter")
...
export async function fetchAdminStats() {
  try {
    ...
    if (result.success && result.data) {
      ...
      return {
        productCount: result.data.productCount || result.data.totalProducts || 0,
        contentCount: result.data.contentCount || result.data.totalContents || 0,
